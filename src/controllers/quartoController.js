...
export const uploadFoto = async (req, res) => {
    try {
        ...
        if (!req.file) {
            return res.status(400).json({ error: 'Nenhum arquivo enviado. Envie uma foto para o quarto!' });
        }

        const quarto = await QuartosModel.buscarPorId(parseInt(id));
        if (!quarto) {
            removerFoto(req.file.path);
            return res.status(404).json({ error: 'Quarto não encontrado para associar a foto.' });
        }
        ...
        return res.status(200).json({ message: 'Foto atualizada com sucesso!', foto: caminhoProcessado });
    } catch (error) {
...
export const criar = async (req, res) => {
    try {
        ...
        const { nome, estado, preco } = req.body;
            ...
            if (!nome){
                return res.status(400).json({ error: 'O campo "nome" é obrigatório para criar um quarto!' });
            }
